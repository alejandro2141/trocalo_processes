...
try { 
html_template = await readHTMLFile(__dirname+"/00010_notification_proposal_rejected_source.html")
//  STEP 1 Get New Proposals NOT YET notified to SOURCE. 
let proposals  = await getProposals()

if (proposals == null || proposals.length < 1 )
{
  console.log (cdate.toLocaleString()+":S00010:INFO: 0 EXIT  No NEW Proposals to Notify Destination ") ; 
  process.exit()
}
...
let emails = await users.map(val => val.email)
  ...
  {
    console.log (cdate.toLocaleString()+":S00010:INFO: notification_new_proposal_source :"+JSON.stringify(emails) )
  ...
  console.log (cdate.toLocaleString()+":S00010:INFO: 0 EXIT ")
  process.exit()
  }
...
async function  getProposals()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE proposal SET notification_rejected_proposal_source  = true  WHERE status = 300  AND ( notification_rejected_proposal_source IS NULL OR  notification_rejected_proposal_source = false )  RETURNING * ; "  
...
async function sendmail(data)
  {
    ...
    let nodemailer = require("nodemailer");
       ...
       // console.log(" Sending Email data :"+JSON.stringify(data))
        ...
        // send some mail
       console.log(cdate.toLocaleString()+":S00010:INFO:EMAILS to send:"+data.email.toLowerCase() )
        ...
        transporter.sendMail(
          {            
            ...
            subject: 'Reusar - Tu propuesta de Intercambio fue Rechazada',
            html: data.message ,
            ses: {
          ...
          (info) => {
            console.log(cdate.toLocaleString()+":S00010:INFO:"+info);
          }
        );
...
async function buildHtmlMessage(html, proposal, user_creator, user_destination){
  ...
  let creator_user_name = user_creator[0].names  
  let destination_user_name = user_destination[0].names  
