...
try { 
html_template = await readHTMLFile(__dirname+"/0002_notification_new_proposal_source.html")
//  STEP 1 Get New Proposals NOT YET notified to SOURCE. 
let proposals  = await getProposals()

if (proposals == null || proposals.length < 1 )
{
  console.log (cdate.toLocaleString()+":S0002:INFO: 0 EXIT  No NEW Proposals to Notify Source ") ; 
  process.exit()
}
...
{
  console.log (cdate.toLocaleString()+":S0002:INFO: notification_new_proposal_source :"+JSON.stringify(emails) )
...
{
  console.log(cdate.toLocaleString()+":S0002:CATCH ERROR PROCESS EXIT:"+e);
  process.exit()
}
...
async function  getProposals()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE proposal SET notification_new_proposal_source = true WHERE notification_new_proposal_source IS NULL OR  notification_new_proposal_source=false  RETURNING *   ;   " ;  
...
async function buildHtmlMessage(html, proposal, user_creator, user_destination){
  ...
  let aux = await html.replace(/\[FRONT_HOST\]/g,FRONT_HOST).replace(/\[PROPOSAL_TITLE\]/g,proposal_title).replace(/\[USER_DESTINATION\]/g, destination_user_name )
