...
html_template = await readHTMLFile(__dirname+"/0001_register_send_email_confirmation.html")
//  STEP 1 Get appointments require recover appointments taken
let emails = await getEmailsRegisterConfirmation()

if (emails != null && emails.length > 0 )
{
  console.log (cdate.toLocaleString()+":S0001:INFO:SEND PROFESSIONAL REGISTER CONFIRMATION :"+JSON.stringify(emails) )
...
console.log (cdate.toLocaleString()+":S0001:INFO: 0 EXIT ")
process.exit()
}
...
async function  getEmailsRegisterConfirmation()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE user_created SET register_confirmation_sent = true WHERE register_confirmation_sent IS NULL OR  register_confirmation_sent=false  RETURNING *   ;   " ;  
       ...
       console.log(cdate.toLocaleString()+":S0007:INFO:EMAILS to send:"+data.email.toLowerCase() )
        ...
        transporter.sendMail(
          {            
            ...
            subject: 'Reusar - Tu cuenta ya esta disponible. Vamos a Intercambiar',
            html: data.message ,
