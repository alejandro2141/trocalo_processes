...
try { 
html_template = await readHTMLFile(__dirname+"/0007_notification_proposal_inTransfer_source.html")
//  STEP 1 Get New Proposals NOT YET notified to SOURCE. 
let proposals  = await getProposals()

if (proposals == null || proposals.length < 1 )
{
  console.log (cdate.toLocaleString()+":S0007:INFO: 0 EXIT  No NEW Proposals to Notify Destination ") ; 
  process.exit()
}
...
let emails = await users.map(val => val.email)
  ...
  {
    console.log (cdate.toLocaleString()+":S0007:INFO: notification in transfer :"+JSON.stringify(emails) )
...
async function  getProposals()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE proposal SET notification_intransfer_proposal_source  = true  WHERE status = 200  AND ( notification_intransfer_proposal_source IS NULL OR  notification_intransfer_proposal_source = false )  RETURNING * ; "  
