...
try { 
html_template = await readHTMLFile(__dirname+"/0003_notification_new_proposal_destination.html")
//  STEP 1 Get New Proposals NOT YET notified to SOURCE. 
let proposals  = await getProposals()

if (proposals == null || proposals.length < 1 )
{
  console.log (cdate.toLocaleString()+":S0003:INFO: 0 EXIT  No NEW Proposals to Notify Destination ") ; 
  process.exit()
}
...
{
  console.log (cdate.toLocaleString()+":S0003:INFO: notification_new_proposal_destination :"+JSON.stringify(emails) )
...
async function  getProposals()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE proposal SET notification_new_proposal_destination = true WHERE notification_new_proposal_destination IS NULL OR  notification_new_proposal_destination = false  RETURNING *   ;   " ;  
