...
try { 
html_template = await readHTMLFile(__dirname+"/0004_notification_proposal_accepted_destination.html")
//  STEP 1 Get New Proposals NOT YET notified to SOURCE. 
let proposals  = await getProposals()

if (proposals == null || proposals.length < 1 )
{
  console.log (cdate.toLocaleString()+":S0004:INFO: 0 EXIT  No NEW Proposals to Notify Destination ") ; 
  process.exit()
}
...
let emails = await users.map(val => val.email)
  ...
  {
    console.log (cdate.toLocaleString()+":S0004:INFO: notification_new_proposal_destination :"+JSON.stringify(emails) )
      ...
      for (let i = 0; i < proposals.length ; i++) {
          ...
          let register = { 
                  'email' : user_destination[0].email , 
                  'message' : "<h1></h1>"
              }
...
{
  console.log(cdate.toLocaleString()+":S0004:CATCH ERROR PROCESS EXIT:"+e);
  process.exit()
}
...
async function  getProposals()
{
  ...
  await client.connect()
      ...
      const sql_calendars  = "UPDATE proposal SET notification_accepted_proposal_destination  = true  WHERE status = 100  AND ( notification_accepted_proposal_destination IS NULL OR  notification_accepted_proposal_destination = false )  RETURNING * ; "  
...
async function sendmail(data)
  {
    ...
    let nodemailer = require("nodemailer");
       ...
       // console.log(" Sending Email data :"+JSON.stringify(data))
        ...
        // send some mail
       console.log(cdate.toLocaleString()+":S0004:INFO:EMAILS to send:"+data.email.toLowerCase() )

       
        transporter.sendMail(
          {            
            //from: "Registro_Horapo_"+Math.floor(Math.random()* (1000 - 1) + 1)+"@horapo.com",
            from: "noreply@horapo.com",
            to: data.email.toLowerCase()  ,
            subject: 'Reusar - Has recibido una propuesta de Intercambio',
            html: data.message ,
            ses: {
          ...
          (info) => {
            console.log(cdate.toLocaleString()+":S0004:INFO:"+info);
          }
        );
